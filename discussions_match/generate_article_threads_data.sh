...
article_ids_titles='../../talk_files/Article_ids_titles.csv'
cache_dir='cache'
...
do 
    ...
    `echo "SELECT r.id, r.user, r.timestamp, r.hash, REPLACE(r.comment, '\n', ' ') AS comment FROM revisions r, article a, article_revisions ar WHERE r.id = ar.revision_id AND ar.article_id = a.id AND a.title = '${page}' INTO OUTFILE '/tmp/revisions.tsv' FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n'" | mysql -u $MYSQLUSER -p$MYSQLPASS $MYSQLDB`
    ...
    # @todo, ! -s does not allow for update
